/**
 * ComfyUIVideoProvider — local video generation via ComfyUI + AnimateDiff.
 * Requires: ComfyUI-AnimateDiff-Evolved + ComfyUI-VideoHelperSuite
 * Setup: https://github.com/Kosinkadink/ComfyUI-AnimateDiff-Evolved
...
function buildAnimateDiffWorkflow(prompt: string, negativePrompt: string, steps: number, cfgScale: number, width: number, height: number, seed: number, numFrames: number): Record<string, unknown> {
  return {
    "1": { class_type: "CheckpointLoaderSimple", inputs: { ckpt_name: "v1-5-pruned-emaonly.ckpt" } },
    "2": { class_type: "CLIPTextEncode", inputs: { text: prompt, clip: ["1", 1] } },
    "3": { class_type: "CLIPTextEncode", inputs: { text: negativePrompt || "ugly, deformed, watermark, blurry", clip: ["1", 1] } },
    "4": { class_type: "ADE_AnimateDiffLoaderWithContext", inputs: { model_name: "mm_sd_v15_v2.ckpt", model: ["1", 0], beta_schedule: "sqrt_linear (AnimateDiff)" } },
    "5": { class_type: "EmptyLatentImage", inputs: { width, height, batch_size: numFrames } },
    "6": { class_type: "KSampler", inputs: { seed, steps, cfg: cfgScale, sampler_name: "euler", scheduler: "normal", denoise: 1, model: ["4", 0], positive: ["2", 0], negative: ["3", 0], latent_image: ["5", 0] } },
    "7": { class_type: "VAEDecode", inputs: { samples: ["6", 0], vae: ["1", 2] } },
    "8": { class_type: "VHS_VideoCombine", inputs: { images: ["7", 0], frame_rate: 8, loop_count: 0, filename_prefix: "jarvis3_video", format: "video/h264-mp4", pingpong: false, save_output: true } },
  };
}
...
interface ComfyUIHistoryItem {
  ...
  outputs: Record<string, { videos?: Array<{ filename: string; subfolder: string; type: string }>; images?: Array<{ filename: string; subfolder: string; type: string }>; gifs?: Array<{ filename: string; subfolder: string; type: string }> }>;
}
...
export class ComfyUIVideoProvider extends BaseProvider {
  ...
  async generate(input: JobInput, opts: GenerateOptions): Promise<JobOutput[]> {
    ...
    opts.onProgress?.(10);
    const workflow = (options.workflow as Record<string, unknown>) ?? buildAnimateDiffWorkflow(input.prompt, negativePrompt, steps, cfgScale, width, height, seed, numFrames);
    ...
    opts.onProgress?.(20);
    const outputFiles = await this.pollForCompletion(queued.prompt_id, opts.onProgress);
    opts.onProgress?.(80);
    ...
    for (const fileInfo of outputFiles) {
      ...
      const fileRes = await fetch(`${this.endpoint}/view?filename=${encodeURIComponent(fileInfo.filename)}&subfolder=${encodeURIComponent(fileInfo.subfolder)}&type=${fileInfo.type}`, { signal: AbortSignal.timeout(60000) });
      ...
      outputs.push({ type: "video", filePath: `/outputs/videos/${filename}`, absolutePath, mimeType: isVideo ? "video/mp4" : "image/gif", metadata: { prompt: input.prompt, seed, steps, numFrames, width, height, provider: this.id } });
    }
    opts.onProgress?.(100);
    return outputs;
  }
    ...
    for (let i = 0; i < 600; i++) {
      ...
      try {
        ...
        if (item.status.completed) {
          ...
          for (const o of Object.values(item.outputs)) {
            if (o.videos) files.push(...o.videos);
            else if (o.gifs) files.push(...o.gifs);
            else if (o.images) files.push(...o.images);
          }
          return files;
        }
        if (item.status.status_str === "error") throw new Error("ComfyUI video generation failed");
      } catch (e) { if (e instanceof Error && e.message.includes("ComfyUI")) throw e; }
      onProgress?.(Math.round(20 + Math.min(55, (i / 600) * 55)));
    }
